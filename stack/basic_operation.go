...
// swap top 2 node
func swap(head *model.Node) *model.Node {
...
func push(srcHead *model.Node, destHead *model.Node) (*model.Node, *model.Node) {
...
func rotate(head *model.Node) *model.Node {
	if head == nil || head.NextNode == nil {
		return head
	}
...
func reverseRotate(head *model.Node) *model.Node {
	if head == nil || head.NextNode == nil {
		return head
	}
...
func CreateStack(intSlice []int) *model.Node {
	var head *model.Node
	...
	for i := len(intSlice) - 1; i >= 0; i-- {
		...
		head = &model.Node{Value: intSlice[i], NextNode: head}
	}
